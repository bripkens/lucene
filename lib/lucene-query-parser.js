...
module.exports = (function(){
  ...
  var result = {
    ...
    parse: function(input, startRule) {
      var holdingBay = {}, uniqueId = String(Math.floor((1 + Math.random()) * 0x10000));

      var unMangleTerm = function unMangleTerm(term) {
        Object.keys(holdingBay).forEach(function (key) {
          var chars = holdingBay[key];
          var pattern = new RegExp('(' + key + ')', 'g');
          term = term.replace(pattern, chars);
        });
        term = term.replace(new RegExp('.empty' + uniqueId, 'g'), '');
        return term;
      };

      var pattern = new RegExp(LUCENE_RESERVED_CHARS.map(function (char) {
        return '(\\\\\\' + char + ')';
      }).join('|'), 'g');

      // For now, filter out escaped characters that the parser has trouble with,
      // and rebuild the terms later.
      input = input.replace(pattern, function (match) {
        var key = "__" + window.btoa(match) + "__";
        holdingBay[key] = match;
        return key;
      });

      // Mangle empty quotes
      input = input.replace(new RegExp('""', 'g'), '.empty' + uniqueId);
      ...
      function parse_term() {
        ...
        if (result0 !== null) {
          result0 = (function(offset, op, term, proximity, boost) {
                var result = { 'term': unMangleTerm(term) };
