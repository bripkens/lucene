...
module.exports = (function(){
  ...
  var result = {
    ...
    parse: function(input, startRule) {
      ...
      const unMangleTerm = (term) => {
        Object.keys(holdingBay).forEach((key) => {
          const chars = holdingBay[key];
      ...
      const pattern = new RegExp(LUCENE_RESERVED_CHARS.map((char) => {
        return `(\\\\\\${char})`;
      }).join('|'), 'g');
      ...
      input = input.replace(pattern, (match) => {
        const key = `__${window.btoa(match)}__`;
